...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (pageId) {
      ...
      // TEMPORARY DEBUG: if the create fails, also fetch the resolved data
      // source's actual schema so the error shows exactly what property
      // names Notion thinks this data source has — removes all guesswork
      // about ID/name mismatches. Safe to leave in; only fires on failure.
      if (!notionRes.ok) {
        try {
          const schemaRes = await fetch(`https://api.notion.com/v1/data_sources/${dataSourceId}`, { headers });
          const schemaData = await schemaRes.json();
          const data = await notionRes.json();
          res.status(notionRes.status).json({
            error: data.message || 'Notion API error',
            details: data,
            debug_resolvedDataSourceId: dataSourceId,
            debug_actualPropertyNames: schemaData.properties ? Object.keys(schemaData.properties) : schemaData
          });
          return;
        } catch (debugErr) {
          // fall through to normal error handling below if the debug fetch itself fails
        }
      }
    }
